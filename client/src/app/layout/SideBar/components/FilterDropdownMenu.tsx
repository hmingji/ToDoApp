...
import {
  removeTaskItemParams,
  resetTaskItemParams,
  setTaskItemParams,
} from '../../../../features/ToDoList/taskItemSlice';
...
export default function FilterDropdownMenu({ ifMenuOpen, darkMode }: Props) {
  ...
  function addLabelFilter(label: string) {
    if (!taskItemParams.labels.includes(label)) {
      dispatch(
        setTaskItemParams({ labels: [...taskItemParams.labels, label] })
      );
    }
  }
  ...
  function removeLabelFilter(label: string) {
    dispatch(
      setTaskItemParams({
        labels: [...taskItemParams.labels.filter((item) => item !== label)],
      })
    );
  }
  ...
  function addPriorityFilter(priority: string) {
    if (!taskItemParams.priorities.includes(priority)) {
      dispatch(
        setTaskItemParams({
          priorities: [...taskItemParams.priorities, priority],
        })
      );
    }
  }
  ...
  function removePriorityFilter(priority: string) {
    dispatch(
      setTaskItemParams({
        priorities: [
          ...taskItemParams.priorities.filter((item) => item !== priority),
        ],
      })
    );
  }
  ...
  return (
    <CSSTransition
      ...
      unmountOnExit
    >
      <div ref={filterMenuNodeRef} style={{ width: 300, marginLeft: '1.5rem' }}>
        <FilterChecklist
        ...
        <DesktopDatePicker
          ...
          renderInput={({ inputRef, inputProps, InputProps }) => (
            ...
            >
              ...
              <IconButton onClick={() => setDatepickerOpen(!datepickerOpen)}>
